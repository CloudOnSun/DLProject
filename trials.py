...
def get_train_test_loaders_60(train_tfms, test_tfms):
    overfit_ids = read_ids("ids/train_ids_subsample_60.txt")
    print("Loaded", len(overfit_ids), "train samples")
    if len(overfit_ids) == 0:
        raise RuntimeError("train_ids_subsample.txt is empty. Check how you generated it.")

    # test split
    test_ids = read_ids("ids/test_ids_subsample_60.txt")
    print("Loaded", len(test_ids), "test samples")
    if len(test_ids) == 0:
        raise RuntimeError("test_ids_subsample.txt is empty.")
...
def trial_boundary_resnet_50(train_loader, test_loader, dataset_split, trial_name, trial_id):
    print("Running trial " + trial_name + " on dataset " + dataset_split + " id " + trial_id)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Using device:", device)
...
def trial_with_find_lr_boundary(train_loader, test_loader, dataset_split, trial_name, trial_id):
    print("Running trial " + trial_name + " on dataset " + dataset_split + " id " + trial_id)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Using device:", device)
...
if __name__ == "__main__":
    train_loader, test_loader = get_train_test_loaders(tfms_building_focus_train, tfms_val_normalized_encoder_vals)
    trial_boundary(train_loader, test_loader, "train_test_split", "found_building_focus_p_09_lr_005", "39")